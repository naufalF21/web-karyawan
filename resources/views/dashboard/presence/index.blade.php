            ...
            <main>
                <div class="container-fluid">
                        ...
                        <div class="col-sm-6 col-md-8">
                            <h3 class="">
                                ...
                                <small class="h6 text-primary">
                                    ...
                                    {{ Route::is('presence') ? $users->count() : $absens->count() }}
                                </small>
                            </h3>
                        </div>
                    </div>
                    ...
                    <ul class="nav nav-tabs mt-4">
                        <li class="nav-item">
                            <a class="nav-link active text-primary" aria-current="page" href="{{ route('presence') }}">All
                                Employees</a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link text-secondary" href="{{ route('presence.attended') }}">Attended</a>
                        </li>
                    </ul>
                    {{-- end tab --}}
                    <div class="card mb-4 ">
                        <div class="d-flex flex-row w-100 justify-content-between align-items-center pt-3 px-3">
                            ...
                            <div class="dropdown">
                                ...
                                <form class="dropdown-menu p-4" action="{{ route('presence.filter', ['page' => 'index']) }}"
                                    method="post">
                                    @csrf
                                    <div class="mb-3">
                                        ...
                                        @else
                                            @foreach ($users as $user)
                                                <tr>
                                                    ...
                                                    <td>
                                                        @if ($absen->getStatus($user['id']) > 0)
                                                            @if ($absen->getStatus($user['id']) == 'Attended')
                                                                <span class="text-success">
                                                                    {{ $absen->getStatus($user['id']) }}
                                                                </span>
                                                            @else
                                                                <span class="text-danger">
                                                                    {{ $absen->getStatus($user['id']) }}
                                                    ...
                                                    <td>
                                                        @if ($absen->getStatus($user['id']) > 0)
                                                            -
                                                        @else
                                                            <form action="{{ route('presence.absent', $user['id']) }}"
                                                                method="post">
                                                                @csrf
                                                                <button type="submit"
